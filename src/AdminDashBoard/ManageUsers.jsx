...
const getRoleTag = (role) => {
  ...
  switch (role) {
    case "donor":
      return <span className={`${base} bg-red-100 text-red-600`}><User size={14} /> Donor</span>;
    case "volunteer":
      return <span className={`${base} bg-blue-100 text-blue-600`}><Users size={14} /> Volunteer</span>;
    case "admin":
...
export default function AllUsers() {
  ...
  // Fetch current user's role (not from useQuery — only once)
  useEffect(() => {
  ...
  useEffect(() => {
    if (!checking && role && role !== "admin") {
      navigate("/forbidden");
    }
  }, [checking, role, navigate]);
